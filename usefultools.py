...
def bqbzz(openimg, size=30, rotate=0, color=(0, 0, 0), ttf=None, text="我\n真\n是\n个\n大\n聪\n明"):
    ...
    plt.imshow(img)
    plt.show()
def json_rw(filename,mode='r',enc='utf-8'):
...
def finder(lst,wtg,dev=False):#ver1.1.5:append "dev mode" for developers
    ...
    l = len(lst)
    il = int(len(wtg))
    # Warning:There is a problem with this line of code. Please submit a pull request on GitHub(for i in range(0,l,step=il):
